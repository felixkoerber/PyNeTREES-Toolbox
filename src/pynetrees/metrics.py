...
from ._population import accepts_population, is_population, require_population
...
def _descendants_of(blocks, node: int) -> np.ndarray:
    ...
    :func:`~pynetrees.sub_tree` returns, looked up in O(subtree size) from the
    prebuilt :func:`~pynetrees.graphtheory._subtree_blocks` decomposition.
...
@accepts_population
def cyl_tree(tree: Tree, dim: int | None = None):
...
@accepts_population
def len_tree(tree: Tree, dim: int | None = None) -> np.ndarray:
...
@accepts_population
@empty_safe("zero")
def L_tree(tree: Tree, dim: int | None = None) -> float:
    ...
    """
    return float(len_tree(tree, dim).sum())
...
def bin_tree(tree: Tree | list[Tree], v: np.ndarray | None = None, bins=10):
    ...
    **A list of trees** returns ``(bin_indices, edges)`` where
    ...
    if is_population(tree):
        tree = require_population(tree, "bin_tree")
        values = [eucl_tree(t) for t in tree] if v is None else list(v)
        bins_arr = np.asarray(bins, dtype=float)
        if bins_arr.ndim == 0:
            pooled = np.concatenate(values)
            bins = np.linspace(pooled.min(), pooled.max() * 1.0001,
                               int(bins_arr) + 1)
        return ([bin_tree(t, each, bins)[0] for t, each in zip(tree, values)],
                np.asarray(bins, dtype=float))

    v = eucl_tree(tree) if v is None else np.asarray(v, dtype=float)
    if len(v) == 0:
        # no nodes to bin, and `v.min()` below has no identity
...
# ---------------------------------------------------------------------------
# scaling a tree to a target size (V4)
# ---------------------------------------------------------------------------
...
class Scaled(NamedTuple):
    ...
    """The rescaled tree."""
    factor: float
    """The linear factor applied, so ``scale_tree(tree, factor)`` repeats it."""
    error: float
    """Relative miss: ``achieved / target - 1``. Zero would be exact."""


@accepts_population(paired="target")
@empty_safe(lambda tree: Scaled(tree, 1.0, float("nan")))
def scaleS_tree(tree: Tree, target: float = 1_000_000.0, *,
                radius: int | None = None, resample: bool = True,
                method: str = "close", shrink: float = 0.5) -> Scaled:
    ...
    """
    from .density import span_tree

    factor = 1.0
    scaled = tree
    for _ in range(2):
        area = span_tree(scaled, radius, method=method, shrink=shrink).area
        if area <= 0:
            raise ValueError(
                f"tree {tree.name!r} spans no area, so it cannot be scaled to "
                f"{target} um^2"
            )
        step = float(np.sqrt(target / area))
        scaled = scale_tree(scaled, step)
        factor *= step

    if resample:
        from .edit import resample_tree

        scaled = resample_tree(scaled, 1.0)

    achieved = span_tree(scaled, radius, method=method, shrink=shrink).area
    return Scaled(scaled, factor, float(achieved / target - 1.0))


@accepts_population(paired="target")
@empty_safe(lambda tree: Scaled(tree, 1.0, float("nan")))
def scaleV_tree(tree: Tree, target: float = 1_000_000.0, *,
                shrink: float = 0.5, dim: int | None = None) -> Scaled:
    ...
    """
    from .density import boundary_tree

    dim = resolve_dim(dim)
    enclosed = boundary_tree(tree, shrink=shrink, dim=dim).volume
    if enclosed <= 0:
        raise ValueError(
            f"tree {tree.name!r} encloses no volume, so it cannot be scaled "
            f"to {target}"
        )

    factor = float((target / enclosed) ** (1.0 / dim))
    scaled = scale_tree(tree, factor)
    achieved = boundary_tree(scaled, shrink=shrink, dim=dim).volume
    return Scaled(scaled, factor, float(achieved / target - 1.0))
