...
def _empty_grid(tree):
    """A density grid of nothing: no voxels, no axes."""
    blank = np.empty(0)
    return DensityGrid(np.empty((0, 0, 0)), blank, blank.copy(), blank.copy())

def _empty_hull(tree):
    return HullResult(vertices=np.empty((0, 3)), faces=np.empty((0, 3), int),
                      polygons=None, distances=None,
                      grid=(np.empty(0), np.empty(0), np.empty(0)))

def _empty_voronoi(tree):
    return VoronoiResult(volumes=np.empty(0), regions=[])

def _empty_boundary(tree):
    return Boundary(vertices=np.empty((0, 3)), faces=np.empty((0, 3), int),
                    volume=0.0, points=np.empty((0, 3)),
                    simplices=np.empty((0, 4), int), polygon=None)
...
@accepts_population
def gdens_tree(tree, sr: float = 5.0, nodes=None) -> DensityGrid:
    ...
    if isinstance(tree, Tree):
        ...
        points = np.atleast_2d(np.asarray(tree, dtype=float))
        if points.shape[1] == 2:  # a planar cloud still needs a z axis
    ...
    if len(X) == 0:
        # an empty tree, or an empty point cloud -- there is nothing to bin,
        # and `a.min()` below has no identity to fall back on
        return _empty_grid(None)
...
@accepts_population
@empty_safe(_empty_hull)
def hull_tree(tree: Tree, thr: float = 25.0, bx=50, by=50, bz=50,
              dim: int | None = None, *, return_distances: bool = False) -> HullResult:
    ...
    try:
        ...
        raise ImportError(
            ...
            "with `pip install pynetrees[plot]` (2D works without it)"
        ) from None
...
# ---------------------------------------------------------------------------
# spanned area and space filling (V4)
# ---------------------------------------------------------------------------
...
class Span(NamedTuple):
    ...
    mask: np.ndarray
    """``(side, side)`` bool, indexed ``[y, x]``, True inside the span."""
    area: float
    """Number of True pixels, i.e. the spanned area [um^2] at 1 um/pixel."""
    origin: int
    """Row/column of the coordinate origin: ``mask[y + origin, x + origin]``."""


def _disk_close(image: np.ndarray, radius: int) -> np.ndarray:
    """Morphological closing by a Euclidean disk, via distance transforms.

    Dilating by a disk of radius ``r`` keeps every pixel within ``r`` of the
    set, which is one Euclidean distance transform; eroding it is another.
    The equivalent footprint convolution is ``O(n^2 r^2)`` -- on a
    900x900 image with ``r = 150`` that is 7e10 operations, versus 8e5 here
    -- and gives the identical answer, which ``tests/test_span.py`` checks
    against the literal definition on images where both are cheap.

    The erosion is padded with True so that a pixel whose disk hangs off
    the edge of the image is not eroded for it, which is what
    ``imerode``/``binary_erosion`` do by default. Without the pad the two
    implementations part company along the border -- unreachable through
    :func:`span_tree`, which leaves a margin of ``radius``, but the point
    of the fast path is that it is the same function, not nearly.
    """
    from scipy import ndimage

    if radius <= 0:
        return image.astype(bool)
    dilated = ndimage.distance_transform_edt(~image) <= radius
    padded = np.pad(dilated, radius, mode="constant", constant_values=True)
    eroded = ndimage.distance_transform_edt(padded) > radius
    return eroded[radius:-radius, radius:-radius]


def _default_radius(tree: Tree) -> int:
    """MATLAB's rule: half the geometric mean of the XY extent."""
    span_x = float(tree.X.max() - tree.X.min())
    span_y = float(tree.Y.max() - tree.Y.min())
    return int(round(np.sqrt(span_x * span_y) / 2))


def _empty_span(tree):
    """No nodes, so no territory: an area of zero, and a mask with nothing
    in it rather than a 1x1 image of the origin."""
    return Span(mask=np.zeros((0, 0), dtype=bool), area=0.0, origin=0)


@accepts_population
@empty_safe(_empty_span)
def span_tree(tree: Tree, radius: int | None = None, *, method: str = "close",
              shrink: float = 0.5) -> Span:
    ...
    **The disk differs from MATLAB's.** MATLAB closes with
    ...
    """
    if method not in ("close", "hull"):
        raise ValueError(f"method must be 'close' or 'hull', got {method!r}")

    radius = _default_radius(tree) if radius is None else int(radius)
    origin = int(round(max(
        max(abs(tree.X.min()), tree.X.max()),
        max(abs(tree.Y.min()), tree.Y.max()),
    ))) + radius
    side = 2 * origin + 1

    rows = np.round(tree.Y).astype(int) + origin
    cols = np.round(tree.X).astype(int) + origin
    image = np.zeros((side, side), dtype=bool)
    image[rows, cols] = True

    if method == "hull":
        from matplotlib.path import Path

        outline = boundary_tree(tree, shrink=shrink, dim=2)
        if len(outline.vertices) < 3:
            mask = image
        else:
            yy, xx = np.mgrid[0:side, 0:side]
            polygon = Path(np.column_stack([
                np.round(outline.vertices[:, 1]) + origin,
                np.round(outline.vertices[:, 0]) + origin,
            ]))
            mask = polygon.contains_points(
                np.column_stack([yy.ravel(), xx.ravel()])
            ).reshape(side, side)
    else:
        mask = _disk_close(image, radius)

    return Span(mask=mask, area=float(mask.sum()), origin=origin)


@accepts_population
@empty_safe("nan")
def theta_tree(tree: Tree, radius: int | None = None, *,
    ...
    **MATLAB returns an index where this returns a distance.** Its
    ...
    seeds = (~span.mask).copy() if include_boundary else np.zeros_like(span.mask)
    seeds[rows, cols] = True
    distance = ndimage.distance_transform_edt(~seeds)

    inside = distance[span.mask]
    if inside.size == 0:
        return float("nan")

    # Bins of one micron centred on 0, 1, 2, ..., matching MATLAB's `histax`
    edges = np.arange(-0.5, np.ceil(inside.max()) + 1.0, 1.0)
    covered = np.cumsum(np.histogram(inside, bins=edges)[0])
    reached = np.flatnonzero(covered > 0.9069 * span.area)
    return float(reached[0]) if len(reached) else float(np.ceil(inside.max()))


class ThetaMC(NamedTuple):
    """Result of :func:`theta_mc_tree`."""

    theta: float
    """Radius [um] covering 90.69% of the enclosed volume."""
    distances: np.ndarray
    """Distance from each sampled point to its nearest node [um]."""


@accepts_population
@empty_safe(lambda tree: ThetaMC(float("nan"), np.empty(0)))
def theta_mc_tree(tree: Tree, n: int = 100_000, alpha: float = 0.5, *,
                  dim: int | None = None, rng=None) -> ThetaMC:
    ...
    **Only compare cells sampled at the same rate.** This measures inside
    ...
    if len(coords) < dim + 1:
        return ThetaMC(float("nan"), np.empty(0))

    reference = _alpha_shape(coords, alpha)
    if reference.volume <= 0:
        return ThetaMC(float("nan"), np.empty(0))

    samples = _sample_in_simplices(reference.points, reference.simplices,
                                   n, generator)
    distances, _ = cKDTree(coords).query(samples)
    return ThetaMC(float(np.quantile(distances, 0.9069)), distances)
