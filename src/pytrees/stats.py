...
def bf_tree(
    ...
    *,
    ...
    (``params``) suggested. The old spelling still works and warns.
    """
    if params is not None:
        if fit_constants is not None:
            raise ValueError("pass either fit_constants or params, not both")
        warnings.warn(
            "bf_tree(params=) is deprecated; use fit_constants=",
            DeprecationWarning, stacklevel=2,
        )
        fit_constants = params
...
def r_mc_tree(tree: Tree, alpha: float = 0.5, n_mc: int = 100,
    ...
    **Sampling is exact rather than by rejection.** MATLAB fills the
    ...
    ``pytrees.density._sample_in_simplices``).
...
def dissectSholl_tree(tree: Tree, c: float | None = None, dim: int = 3, *,
    ...
    *does*. Reproduced as-is; see MATLAB_TOOLBOX_BUGS.md.
    ...
    if centripetal:
        ...
        bf, k = bf_tree(rootangle, dim=f"{dim}d")
