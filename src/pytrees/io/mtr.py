...
(HDF5) via the optional `mat73` dependency. See :mod:`pytrees.io._matlab`
...
def load_mtr(path: str | Path, variable: str | None = None) -> Tree | list[Tree]:
    ...
    variable : str, optional
        ...
        prefers one literally named ``tree`` (what ``save_tree.m`` writes)
        and otherwise falls back to the sole variable that looks like tree
        data. Name it explicitly when a file holds several candidates.
    ...
    ValueError
        ...
        isn't there, or if several candidates exist and none is called
        ``tree`` (the message lists them so you can pick).

    Notes
    -----
    Earlier versions required the variable to be called exactly ``tree`` and
    raised otherwise. That is fine for files MATLAB's own ``save_tree``
    wrote, but rejects any workspace saved by hand or by T2N -- which
    commonly stores ``tree`` alongside other variables, or under a different
    name entirely.
    """
    path = Path(path)
...
def _select_variable(data: dict, path: Path, variable: str | None):
    ...
    raise ValueError(
        f"{path}: several variables look like tree data ({sorted(candidates)}) "
        f"and none is called 'tree'. Pass variable='<name>' to choose one."
    )
...
def _flatten(raw):
    ...
    certain nesting depths) -- :func:`~pytrees.io._matlab.to_plain` reconciles
...
def _struct_to_tree(struct: dict, name: str) -> Tree:
    """Build a :class:`~pytrees.Tree` from one decoded MATLAB tree struct."""
